...
class Database:
    ...
    def create_task_table(self):
        """Create tasks table"""
        self.cursor.execute("CREATE TABLE IF NOT EXISTS tasks(id integer PRIMARY KEY AUTOINCREMENT, task varchar(50) NOT NULL, due_date DATE, completed BOOLEAN NOT NULL CHECK (completed IN (0, 1)))")
        self.con.commit()

    def create_task(self, task, due_date=None):
        """Create a task"""
        if due_date:
            due_date = datetime.strptime(due_date, '%A %d %B %Y').date()
    ...
    def get_tasks(self):
        ...
        today = date.today()
        today_tasks = self.cursor.execute("SELECT id, task, due_date FROM tasks WHERE completed = 0 AND due_date = ?", (today,)).fetchall()
        delayed_tasks = self.cursor.execute("SELECT id, task, due_date FROM tasks WHERE completed = 0 AND due_date < ?", (today,)).fetchall()
        upcoming_tasks = self.cursor.execute("SELECT id, task, due_date FROM tasks WHERE completed = 0 AND due_date > ?", (today,)).fetchall()
    ...
    def get_task(self, taskid):
        ...
        task = self.cursor.execute("SELECT id, task, due_date FROM tasks WHERE id=?", (taskid,)).fetchone()
        return task